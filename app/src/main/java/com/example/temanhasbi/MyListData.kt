package com.example.temanhasbi
...
import com.example.temanhasbi.databinding.ActivityMyListDataBinding
...
class MyListData : AppCompatActivity(), RecyclerViewAdaptor.dataListener {
    ...
    private fun GetData(){
        Toast.makeText(applicationContext, "Tunggu BEB", Toast.LENGTH_LONG).show()
        val getUserID : String = auth?.getCurrentUser()?.getUid().toString()
        val getReference = database.getReference()
        getReference.child("Admin").child(getUserID).child("Teman Hasbi" +
                "")
    ...
    override fun onDeleteData(data: data_teman?, position: Int){
        ...
        if (getReference != null) {
            getReference.child("Admin").child(getUserID).child("Teman Hasbi").child(data?.key.toString())
                .removeValue()
